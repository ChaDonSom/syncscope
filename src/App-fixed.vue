            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center space-x-4 mt-1">
                  ...
                  <p v-if="!event.synced" class="text-xs text-orange-600">Local changes pending</p>
                  <p v-if="event.synced" class="text-xs text-green-600">Synced to remote</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="h-64 overflow-y-auto space-y-1">
          ...
          <div v-for="log in debugLogs" :key="log.timestamp" class="flex items-start space-x-2">
            <span class="text-gray-500 text-xs flex-shrink-0">
              {{ formatDateFromNumber(log.timestamp) }}
            </span>
            <span
              :class="{
...
const events = computed(() => {
  return syncStore.getAllEvents().sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
})
...
function createTestEvent() {
  ...
  if (createdEvent) {
    addDebugLog("success", `Created local event: ${createdEvent.name}`)
  } else {
...
async function testInstantDBWrite() {
  ...
  try {
    ...
    const newEvent = {
      type: "write" as const,
      name: `Direct InstantDB test at ${new Date().toLocaleTimeString()}`,
      entity: "test",
      change: { action: "direct", data: { test: true } },
    ...
    await db.transact(tx.events[id()].update(newEvent))
...
onMounted(async () => {
  ...
  try {
    ...
    unsubscribe = db.subscribeQuery({ events: {} }, (result: any) => {
      ...
      if (result.data && result.data.events) {
        ...
        const remoteEvents = Object.values(result.data.events) as SyncEvent[]
        ...
        addDebugLog("info", "No events found in InstantDB yet")
...
function addMockData(): void {
  ...
  const mockEvents = [
    {
      type: "write" as const,
      name: "User updated profile information (from Pinia ORM)",
      entity: "user",
      change: { action: "update", data: { profile: true } },
    },
    {
      type: "conflict" as const,
      name: 'Concurrent edit on document "Project Plan"',
      entity: "document",
      change: { action: "conflict", data: { document: "Project Plan" } },
    },
    {
      type: "resolved" as const,
      name: "Merged changes from two clients successfully",
      entity: "document",
      change: { action: "resolve", data: { merged: true } },
    },
  ]
